...
with st.sidebar:
    ...
    tab1, tab2 = st.tabs(["Filtro Dummy", "Filtro Funcionando"])
    with tab1:
        date_range = st.slider("Seleccione el rango de fechas:", value=(min_date, max_date), min_value=min_date, max_value=max_date,
                           format="YYYY-MM-DD", step=timedelta(days=1))
    with tab2:
        date_range = st.date_input("Seleccione el rango de fechas", (today, today), start, end, format="YYYY-MM-DD")
